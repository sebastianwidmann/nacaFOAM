...
class generateBlockMeshDict(object):
    ...
    def generatePoints(self):
        ...
        if len(self.airfoil) == 4:
            ...
            z_t = 5 * self.t * (a0 * np.sqrt(x) + a1 * x + a2 * np.power(x,
                                                                         2) + a3 * np.power(
                x, 3) + a4 * np.power(x, 4))
            ...
            if self.p == 0:
                ...
                z_c = np.where(x < self.p, self.m / np.power(self.p, 2) * (
                        2 * self.p * x - np.power(x, 2)),
                               self.m / np.power(1 - self.p, 2) * (
                                       1 - 2 * self.p + 2 * self.p * x - np.power(
                                   x, 2)))
                dz_c = np.where(x < self.p,
                                2 * self.m / np.power(self.p, 2) * (
                                        self.p - x),
                                2 * self.m / np.power(1 - self.p, 2) * (
                                        self.p - x))
            ...
            r_values = np.array(
                [0.0580, 0.1260, 0.2025, 0.2900, 0.3910, 0.1300, 0.2170,
                 0.3180, 0.4410])
            k1_values = np.array(
                [361.400, 51.640, 15.957, 6.643, 3.230, 51.990, 15.793, 6.520,
                 3.191])
            k2k1_values = np.array(
                [0, 0, 0, 0, 0, 0.000764, 0.00677, 0.0303, 0.1355])

            # Find array index for corresponding PQ-value
            index = np.where(
                digits == (10 * self.airfoil[1] + self.airfoil[2]))
            r = r_values[index]
            k1 = k1_values[index]
            ...
            if self.q == 0:
                z_c = np.where(x < r, k1 / 6 * (
                        np.power(x, 3) - 3 * r * np.power(x, 2) + np.power(r,
                    2) * x * (3 - r)), k1 * np.power(r, 3) / 6 * (1 - x))
                dz_c = np.where(x < r, k1 / 6 * (
                        3 * np.power(x, 2) - 6 * r * x + (3 - r) * np.power(r,
                                                                            2)),
                                -k1 * np.power(r, 3) / 6)
            else:
                z_c = np.where(x < r, k1 / 6 * (
                        np.power(x - r, 3) - k2k1 * x * np.power(1 - r,
                                                                 3) - x * np.power(
                    r, 3) + np.power(r, 3)), k1 / 6 * (k2k1 * np.power(x - r,
                                                                       3) - k2k1 * x * np.power(
                    1 - r, 3) - x * np.power(r, 3) + np.power(r, 3)))
                dz_c = np.where(x < r, k1 / 6 * (
                        3 * np.power(x - r, 2) - k2k1 * np.power(1 - r,
                                                                 3) - np.power(
                    r, 3)), k1 / 6 * (3 * k2k1 * np.power(x - r,
                                                          2) - k2k1 * np.power(
                    1 - r, 3) - np.power(r, 3)))
    ...
    def generateWavefrontObject(self):
        f = open(
            'constant/geometry/naca{}_{}deg.obj'.format(self.foil, self.aoa),
            'w+')
        ...
        for i in range(self.points.shape[0]):
            f.write('v {} {} {} \n'.format(self.points[i][0], self.yMax,
                                           self.points[i][1]))
            f.write('v {} {} {} \n'.format(self.points[i][0], self.yMin,
                                           self.points[i][1]))
    ...
    def writeToFile(self):
        ...
        f.write(
            '           ($aerofoil.xLead    -0.1 $aerofoil.zLead)                               \n')
        f.write(
            '           ($aerofoil.xTrail   -0.1 $aerofoil.zTrail)                              \n')
        f.write(
            '           ($domain.xMax       -0.1 $aerofoil.zTrail)                              \n')
        f.write(
            '                                                                                   \n')
        f.write(
            '           ($aerofoil.xLower   -0.1 $aerofoil.zLower)                              \n')
        f.write(
            '           ($aerofoil.xUpper   -0.1 $aerofoil.zUpper)                              \n')
        f.write(
            '                                                                                   \n')
        f.write(
            '           ($aerofoil.xUpper   -0.1 $domain.zMax)                                  \n')
        f.write(
...
if __name__ == '__main__':
    ...
    if np.deg2rad(args.aoa) <= -np.pi / 2 or np.deg2rad(args.aoa) >= np.pi / 2:
        parser.error(
            'Please enter an angle of attack between ({}, {}) degree.'.format(
                -90, 90))
    # if args.mach < 0:
